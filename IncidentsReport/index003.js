...
async function submitFormResponse() {
  ...
  questionToAnswerMap[INCIDENCETYPE] = _incidenceType;
  questionToAnswerMap[COMMENTS] = _destination;
  questionToAnswerMap[DATE] = _date;
  questionToAnswerMap[INCIDENCETIME] = _time;
...
function inflateIncidenceTypeDiv() {
  var incidenceTypeDiv = document.getElementById('incidenceTypeDiv');
  KASClient.UI.clearElement(incidenceTypeDiv);

  var incidenceTypeTitle = KASClient.UI.getElement('div');
  incidenceTypeTitle.className = 'question-title';
  incidenceTypeTitle.innerText = _strings[_form.questions[INCIDENCETYPE].title];

  var incidenceTypeInput = KASClient.UI.getElement('input');
  incidenceTypeInput.type = 'text';
  incidenceTypeInput.className = 'comment-input';

  if (KASClient.getPlatform() == KASClient.Platform.iOS) {
    KASClient.UI.addCSS(incidenceTypeInput, {
      'padding-left': '13pt',
    });
  }
  incidenceTypeInput.placeholder = _strings['strTapToEnter'];
  incidenceTypeInput.addEventListener('input', function (event) {
    _incidenceType = event.target.value;
    invalidateFooter();
  });

  KASClient.UI.addElement(incidenceTypeTitle, incidenceTypeDiv);
  KASClient.UI.addElement(incidenceTypeInput, incidenceTypeDiv);
}
...
function inflateTimeDiv() {
  ...
  var textQuestionTitle = KASClient.UI.getElement('div');
  textQuestionTitle.className = 'question-title';
  textQuestionTitle.innerText = _strings[_form.questions[INCIDENCETIME].title];
...
function inflateSummaryView() {
  ...
  var incidenceTypeDetailsDiv = KASClient.UI.getElement('div', divAttributes);

  var incidenceTypeDetailsHeader = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-bottom': '0pt',
  });

  incidenceTypeDetailsHeader.className = 'comment-header';
  incidenceTypeDetailsHeader.innerText =
    _strings[_form.questions[INCIDENCETYPE].title];
  KASClient.UI.addElement(
    incidenceTypeDetailsHeader,
    incidenceTypeDetailsDiv
  );

  var incidenceTypeDetailsView = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-top': '5pt',
  });

  var incidenceType = KASClient.UI.getElement('div', {
    color: '#32485f',
    'font-size': '12pt',
    'overflow-wrap': 'break-word',
    'word-wrap': 'break-word',
  ...
  incidenceType.innerHTML = _incidenceType;

  KASClient.UI.addElement(incidenceType, incidenceTypeDetailsView);
  KASClient.UI.addElement(
    incidenceTypeDetailsView,
    incidenceTypeDetailsDiv
  );
  ...
  comments.innerHTML = _destination;
  ...
  var incidencTimeDiv = KASClient.UI.getElement('div', divAttributes);

  var incidencTimeHeader = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-bottom': '0pt',
  });

  incidencTimeHeader.className = 'comment-header';
  incidencTimeHeader.innerText = _strings[_form.questions[INCIDENCETIME].title];
  KASClient.UI.addElement(incidencTimeHeader, incidencTimeDiv);

  var incidencTimeView = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-top': '5pt',
  });

  var incidencTime = KASClient.UI.getElement('div', {
    color: '#32485f',
    'font-size': '12pt',
    'overflow-wrap': 'break-word',
    'word-wrap': 'break-word',
  ...
  incidencTime.innerHTML = _time;

  KASClient.UI.addElement(incidencTime, incidencTimeView);
  KASClient.UI.addElement(incidencTimeView, incidencTimeDiv);
  ...
  KASClient.UI.addElement(incidenceTypeDetailsDiv, summaryView);
  KASClient.UI.addElement(commentsDiv, summaryView);
  KASClient.UI.addElement(dateDiv, summaryView);
  KASClient.UI.addElement(incidencTimeDiv, summaryView);
