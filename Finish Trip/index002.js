...
// checks registration number status
var registrationValid = false;
...
function inflateDestinationDiv() {
  var textQuestionDiv = document.getElementById('destinationDiv');
  KASClient.UI.clearElement(textQuestionDiv);
...
function inflateFooterView() {
  ...
  if (_currentPage == 1) {
    ...
    // check for invalid vehicle registration -> if not then change status of trip
     checkAndChangeTripStatus(_registration);
...
function inflateSummaryView() {
  ...
  endingTripHeader.innerText = _strings[_form.questions[DESTINATION].title];
  ...
  endingTrip.innerHTML = _destination;
  ...
  endTimeHeader.innerText = _strings[_form.questions[TRIPTIME].title];
...
// only end trip for registered vehicle
const checkAndChangeTripStatus = async (vehicleRegistration) => {
  const response = await fetch(
    `https://fleet-management-isaac.herokuapp.com/api/change-trip-ended/${vehicleRegistration}`,
    {
      method: 'PUT'
    }
  );
  if (response.status == 400) {
    return showError("Trip is not in progress");
  } else if (response.status == 404) {
    return showError("Vehicle is not registered");
