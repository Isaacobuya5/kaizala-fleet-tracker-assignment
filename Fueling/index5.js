...
async function submitFormResponse() {
  ...
  questionToAnswerMap[FUELSTATION] = _fuelStation;
  questionToAnswerMap[LITRES] = _litres;
  questionToAnswerMap[COST] = _cost;
  questionToAnswerMap[DATE] = _date;
  questionToAnswerMap[FUELTIME] = _time;
  ...
  function inflateLitresDiv() {
    var LITRESDiv = document.getElementById('litresDiv');
    KASClient.UI.clearElement(LITRESDiv);

    var LITRESTitle = KASClient.UI.getElement('div');
    LITRESTitle.className = 'question-title';
    LITRESTitle.innerText = _strings[_form.questions[LITRES].title];

    var LITRESInput = KASClient.UI.getElement('input');
    LITRESInput.type = 'number';
    LITRESInput.className = 'comment-input';

    if (KASClient.getPlatform() == KASClient.Platform.iOS) {
      KASClient.UI.addCSS(LITRESInput, {
        'padding-left': '13pt',
      });
    }
    LITRESInput.placeholder = _strings['strTapToEnter'];
    LITRESInput.addEventListener('input', function (event) {
      _litres = event.target.value;
      invalidateFooter();
    });

    KASClient.UI.addElement(LITRESTitle, LITRESDiv);
    KASClient.UI.addElement(LITRESInput, LITRESDiv);
  }
  ...
  // FUELSTATION
  function inflateFuelStationDiv() {
    var question = _form.questions[FUELSTATION];

    var questionDiv = document.getElementById("fuelStationDiv");
    ...
    for (i = 0; i < question.options.length; i++) {
      ...
      options.onclick = function () {
        _fuelStation = this.value;
        document.getElementById("SingleSelectOption" + _fuelStation).checked = true;
        invalidateFooter();
      };
  ...
  function inflateTimeDiv() {
    ...
    var textQuestionTitle = KASClient.UI.getElement('div');
    textQuestionTitle.className = 'question-title';
    textQuestionTitle.innerText = _strings[_form.questions[FUELTIME].title];
  ...
  function inflateSummaryView() {
    ...
    LITRESDetailsHeader.innerText =
      _strings[_form.questions[LITRES].title];
    ...
    // COST
    var costDiv = KASClient.UI.getElement('div', divAttributes);

    var costHeader = KASClient.UI.getElement('div', {
      "padding": '14px',
      'padding-bottom': '0pt',
    });

    costHeader.className = 'comment-header';
    costHeader.innerText = _strings[_form.questions[COST].title];
    KASClient.UI.addElement(costHeader, costDiv);

    var costView = KASClient.UI.getElement('div', {
      padding: '14px',
      'padding-top': '5pt',
    });
    ...
    KASClient.UI.addElement(cost, costView);
    KASClient.UI.addElement(costView, costDiv);

    // FUELSTATION
    var fuelStationDiv = KASClient.UI.getElement("div", divAttributes);

    var fuelStationHeader = KASClient.UI.getElement("div", {
      "padding": "14px",
      "padding-bottom": "0pt"
    });

    fuelStationHeader.className = "comment-header";
    fuelStationHeader.innerText = _strings[_form.questions[FUELSTATION].title];
    KASClient.UI.addElement(fuelStationHeader, fuelStationDiv);

    var fuelStationView = KASClient.UI.getElement("div", {
      "padding": "14px",
      "padding-top": "5pt"
    });
    ...
    fuelStation.innerHTML = _strings[_form.questions[FUELSTATION].options[_fuelStation].text];


    KASClient.UI.addElement(fuelStation, fuelStationView);
    KASClient.UI.addElement(fuelStationView, fuelStationDiv);
    ...
    var fuelTimeDiv = KASClient.UI.getElement('div', divAttributes);

    var fuelTimeHeader = KASClient.UI.getElement('div', {
      "padding": '14px',
      'padding-bottom': '0pt',
    });

    fuelTimeHeader.className = 'comment-header';
    fuelTimeHeader.innerText = _strings[_form.questions[FUELTIME].title];
    KASClient.UI.addElement(fuelTimeHeader, fuelTimeDiv);

    var fuelTimeView = KASClient.UI.getElement('div', {
      "padding": '14px',
      'padding-top': '5pt',
    });

    var fueltime = KASClient.UI.getElement('div', {
      "color": '#32485f',
      'font-size': '12pt',
      'overflow-wrap': 'break-word',
      'word-wrap': 'break-word',
    ...
    fueltime.innerHTML = _time;

    KASClient.UI.addElement(fueltime, fuelTimeView);
    KASClient.UI.addElement(fuelTimeView, fuelTimeDiv);
    ...
    KASClient.UI.addElement(fuelStationDiv, summaryView);
    KASClient.UI.addElement(LITRESDetailsDiv, summaryView);
    KASClient.UI.addElement(costDiv, summaryView);
    KASClient.UI.addElement(dateDiv, summaryView);
    KASClient.UI.addElement(fuelTimeDiv, summaryView);
