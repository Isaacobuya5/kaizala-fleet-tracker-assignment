...
function updatePage() {
  ...
  if (_currentPage == 7 && _isLocationNotFetched) {
...
function inflateRegistrationDiv() {
  var registrationDiv = document.getElementById('registerDiv');
  KASClient.UI.clearElement(registrationDiv);

  var registrationTitle = KASClient.UI.getElement('div');
  registrationTitle.className = 'question-title';
  registrationTitle.innerText = _strings[_form.questions[REGISTRATION].title];

  var registrationInput = KASClient.UI.getElement('input');
  registrationInput.type = 'text';
  registrationInput.className = 'comment-input';

  if (KASClient.getPlatform() == KASClient.Platform.iOS) {
    KASClient.UI.addCSS(registrationInput, {
      'padding-left': '13pt',
    });
  }
  registrationInput.placeholder = _strings['strEnterVehicleRegistration'];
  registrationInput.addEventListener('input', function (event) {
    _registration = event.target.value;
    invalidateFooter();
  });

  KASClient.UI.addElement(registrationTitle, registrationDiv);
  KASClient.UI.addElement(registrationInput, registrationDiv);
}

// trip date
function inflateTripDateDiv() {
  var registrationDiv = document.getElementById('tripDateDiv');
  KASClient.UI.clearElement(registrationDiv);

  var registrationTitle = KASClient.UI.getElement('div');
  registrationTitle.className = 'question-title';
  registrationTitle.innerText = _strings[_form.questions[TRIPDATE].title];
  ...
  registrationInput.placeholder = _strings['strTripDate'];
  registrationInput.addEventListener('input', function (event) {
...
function inflateFooterView() {
  ...
  if (_currentPage == 1) {
    ...
    if (_windScreen == '' || _sideMirrors == '' || _rearView == '' || validCommentData == false) {
      nextButtonIsDisabled = true;
    }
...
function inflateSummaryView() {
  var summaryView = document.getElementById('page8');
...
function validComment(comment) {
  let driverComment = comment.trim();
  let valid = /^(Good|Faulty)/i.test(driverComment);
  if (!valid) {
    validCommentData = false;
    return showError('Comment must begin with Good/Faulty');
  }
  validCommentData = true;
  return valid;
}
