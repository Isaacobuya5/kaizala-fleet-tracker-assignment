...
async function submitFormResponse() {
  ...
  questionToAnswerMap[EXTENTDAMAGE] = _extentdamage;
  questionToAnswerMap[COMMENTS] = _comments;
  questionToAnswerMap[DATE] = _date;
  questionToAnswerMap[ACCIDENTTIME] = _time;
  ...
  const registered = await fetch(
    `https://fleet-management-isaac.herokuapp.com/api/get-vehicle/${_registration}`
  );
...
function inflateEXTENTDAMAGEDiv() {
  var extendDamageDiv = document.getElementById('extentdamageDiv');
  KASClient.UI.clearElement(extendDamageDiv);

  var extendDamageTitle = KASClient.UI.getElement('div');
  extendDamageTitle.className = 'question-title';
  extendDamageTitle.innerText = _strings[_form.questions[EXTENTDAMAGE].title];

  var extendDamageInput = KASClient.UI.getElement('input');
  extendDamageInput.type = 'text';
  extendDamageInput.className = 'comment-input';

  if (KASClient.getPlatform() == KASClient.Platform.iOS) {
    KASClient.UI.addCSS(extendDamageInput, {
      'padding-left': '13pt',
    });
  }
  extendDamageInput.placeholder = _strings['strTapToEnter'];
  extendDamageInput.addEventListener('input', function (event) {
    _extentdamage = event.target.value;
    invalidateFooter();
  });

  KASClient.UI.addElement(extendDamageTitle, extendDamageDiv);
  KASClient.UI.addElement(extendDamageInput, extendDamageDiv);
}
...
function inflateTimeDiv() {
  ...
  var textQuestionTitle = KASClient.UI.getElement('div');
  textQuestionTitle.className = 'question-title';
  textQuestionTitle.innerText = _strings[_form.questions[TIME].title];
...
function inflateSummaryView() {
  ...
  var extendDamageDetailsDiv = KASClient.UI.getElement('div', divAttributes);

  var extendDamageDetailsHeader = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-bottom': '0pt',
  });

  extendDamageDetailsHeader.className = 'comment-header';
  extendDamageDetailsHeader.innerText =
    _strings[_form.questions[EXTENTDAMAGE].title];
  KASClient.UI.addElement(
    extendDamageDetailsHeader,
    extendDamageDetailsDiv
  );

  var extendDamageDetailsView = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-top': '5pt',
  });

  var extendDamage = KASClient.UI.getElement('div', {
    color: '#32485f',
    'font-size': '12pt',
    'overflow-wrap': 'break-word',
    'word-wrap': 'break-word',
  ...
  extendDamage.innerHTML = _extentdamage;

  KASClient.UI.addElement(extendDamage, extendDamageDetailsView);
  KASClient.UI.addElement(
    extendDamageDetailsView,
    extendDamageDetailsDiv
  );

  var endingTripDiv = KASClient.UI.getElement('div', divAttributes);

  var endingTripHeader = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-bottom': '0pt',
  });

  endingTripHeader.className = 'comment-header';
  endingTripHeader.innerText = _strings[_form.questions[COMMENTS].title];
  KASClient.UI.addElement(endingTripHeader, endingTripDiv);

  var endingTripView = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-top': '5pt',
  });

  var endingTrip = KASClient.UI.getElement('div', {
    color: '#32485f',
    'font-size': '12pt',
    'overflow-wrap': 'break-word',
    'word-wrap': 'break-word',
  ...
  endingTrip.innerHTML = _comments;

  KASClient.UI.addElement(endingTrip, endingTripView);
  KASClient.UI.addElement(endingTripView, endingTripDiv);
  ...
  var endTimeDiv = KASClient.UI.getElement('div', divAttributes);

  var endTimeHeader = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-bottom': '0pt',
  });

  endTimeHeader.className = 'comment-header';
  endTimeHeader.innerText = _strings[_form.questions[ACCIDENTTIME].title];
  KASClient.UI.addElement(endTimeHeader, endTimeDiv);

  var endTimeView = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-top': '5pt',
  });

  var endTime = KASClient.UI.getElement('div', {
    color: '#32485f',
    'font-size': '12pt',
    'overflow-wrap': 'break-word',
    'word-wrap': 'break-word',
  ...
  endTime.innerHTML = _time;

  KASClient.UI.addElement(endTime, endTimeView);
  KASClient.UI.addElement(endTimeView, endTimeDiv);
  ...
  KASClient.UI.addElement(extendDamageDetailsDiv, summaryView);
  KASClient.UI.addElement(endingTripDiv, summaryView);
  KASClient.UI.addElement(dateDiv, summaryView);
  KASClient.UI.addElement(endTimeDiv, summaryView);
