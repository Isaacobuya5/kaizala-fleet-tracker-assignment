...
async function submitFormResponse() {
  ...
  questionToAnswerMap[ODOMETER] = _ordometer;
  questionToAnswerMap[DUEDATE] = _service_due_date;
  ...
  questionToAnswerMap[SERVICETIME] = _time;
  ...
  const registered = await fetch(
    `https://fleet-management-isaac.herokuapp.com/api/${_registration}`
  );
...
function inflateCurrentOrdometerDiv() {
  var odometerDiv = document.getElementById('odometerDiv');
  KASClient.UI.clearElement(odometerDiv);

  var odometerTitle = KASClient.UI.getElement('div');
  odometerTitle.className = 'question-title';
  odometerTitle.innerText = _strings[_form.questions[ODOMETER].title];

  var odometerInput = KASClient.UI.getElement('input');
  odometerInput.type = 'number';
  odometerInput.className = 'comment-input';

  if (KASClient.getPlatform() == KASClient.Platform.iOS) {
    KASClient.UI.addCSS(odometerInput, {
      'padding-left': '13pt',
    });
  }
  odometerInput.placeholder = _strings['strTapToEnter'];
  odometerInput.addEventListener('input', function (event) {
    _ordometer = event.target.value;
    invalidateFooter();
  });

  KASClient.UI.addElement(odometerTitle, odometerDiv);
  KASClient.UI.addElement(odometerInput, odometerDiv);
}

function inflateServiceDueDateDiv() {
  var textQuestionDiv = document.getElementById('serviceDueDateDiv');
  KASClient.UI.clearElement(textQuestionDiv);
  ...
  textQuestionInput.placeholder = _strings['strTapToEnter'];
  textQuestionInput.addEventListener('input', function (event) {
    _service_due_date = event.target.value;
    invalidateFooter();
  });
...
function inflateGarageDiv() {
  ...
  var questionDiv = document.getElementById("garageDiv");
...
// parts needed
function inflatePartsNeededDiv() {
...
function inflateCommentsDiv() {
  var textQuestionDiv = document.getElementById('commentsDiv');
  KASClient.UI.clearElement(textQuestionDiv);

  var textQuestionTitle = KASClient.UI.getElement('div');
  textQuestionTitle.className = 'question-title';
  textQuestionTitle.innerText = _strings[_form.questions[COMMENTS].title];
...
function inflateTimeDiv() {
  ...
  var textQuestionTitle = KASClient.UI.getElement('div');
  textQuestionTitle.className = 'question-title';
  textQuestionTitle.innerText = _strings[_form.questions[SERVICETIME].title];
...
function inflateQuestions() {
  ...
  inflateCurrentOrdometerDiv();

  inflateServiceDueDateDiv();

  inflateGarageDiv();

  inflatePartsNeededDiv();
...
function inflateSummaryView() {
  ...
  // service due date
  var dueDateDiv = KASClient.UI.getElement('div', divAttributes);

  var dueDateHeader = KASClient.UI.getElement('div', {
    "padding": '14px',
    'padding-bottom': '0pt',
  });

  dueDateHeader.className = 'comment-header';
  dueDateHeader.innerText = _strings[_form.questions[DUEDATE].title];
  KASClient.UI.addElement(dueDateHeader, dueDateDiv);

  var dueDateView = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-top': '5pt',
  });
  ...
  dueDate.innerHTML = _service_due_date;
  

  KASClient.UI.addElement(dueDate, dueDateView);
  KASClient.UI.addElement(dueDateView, dueDateDiv);

  // garage
  var gardgeDetailsDiv = KASClient.UI.getElement("div", divAttributes);

  var gardgeDetailsHeader = KASClient.UI.getElement("div", {
    "padding": "14px",
    "padding-bottom": "0pt"
  });

  gardgeDetailsHeader.className = "comment-header";
  gardgeDetailsHeader.innerText = _strings[_form.questions[GARAGE].title];
  KASClient.UI.addElement(gardgeDetailsHeader, gardgeDetailsDiv);

  var gardgeDetailsView = KASClient.UI.getElement("div", {
    "padding": "14px",
    "padding-top": "5pt"
  });

  var gardge = KASClient.UI.getElement("div", {
    "color": "#32485f",
    "font-size": "12pt",
    "overflow-wrap": "break-word",
    "word-wrap": "break-word",
  ...
  gardge.innerHTML = _strings[_form.questions[GARAGE].options[_garage].text];


  KASClient.UI.addElement(gardge, gardgeDetailsView);
  KASClient.UI.addElement(gardgeDetailsView, gardgeDetailsDiv);


  // parts
  var partsDiv = KASClient.UI.getElement('div', divAttributes);

  var partsHeader = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-bottom': '0pt',
  });

  partsHeader.className = 'comment-header';
  partsHeader.innerText = _strings[_form.questions[PARTS].title];
  KASClient.UI.addElement(partsHeader, partsDiv);

  var partsView = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-top': '5pt',
  });
  ...
  KASClient.UI.addElement(parts, partsView);
  KASClient.UI.addElement(partsView, partsDiv);
  ...
  commentsHeader.innerText = _strings[_form.questions[COMMENTS].title];
  ...
  KASClient.UI.addElement(comments, commentsView);
...
alert("date inserted")
  ...
  var serviceTimeDiv = KASClient.UI.getElement('div', divAttributes);

  var serviceTimeHeader = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-bottom': '0pt',
  });

  serviceTimeHeader.className = 'comment-header';
  serviceTimeHeader.innerText = _strings[_form.questions[SERVICETIME].title];
  KASClient.UI.addElement(serviceTimeHeader, serviceTimeDiv);

  var serviceTimeView = KASClient.UI.getElement('div', {
    padding: '14px',
    'padding-top': '5pt',
  });

  var serviceTime = KASClient.UI.getElement('div', {
    color: '#32485f',
    'font-size': '12pt',
    'overflow-wrap': 'break-word',
    'word-wrap': 'break-word',
  ...
  serviceTime.innerHTML = _time;
alert("time inserted");
  KASClient.UI.addElement(serviceTime, serviceTimeView);
  KASClient.UI.addElement(serviceTimeView, serviceTimeDiv);
  ...
  KASClient.UI.addElement(dueDateDiv, summaryView);
  KASClient.UI.addElement(gardgeDetailsDiv, summaryView);
  KASClient.UI.addElement(partsDiv, summaryView);
  KASClient.UI.addElement(commentsDiv, summaryView);
  KASClient.UI.addElement(dateDiv, summaryView);
  KASClient.UI.addElement(serviceTimeDiv, summaryView);
